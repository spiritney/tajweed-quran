...
const formatAyah = (item: any) => {
  const ayatAsArray = item.trim().split("\n")
  return JSON.stringify(ayatAsArray.map((ayat: any) => ayat.trim()), null, 2);
}
  ...
  for (let surahNumber = 1; surahNumber <= 114; surahNumber++) {
    await (async (surahNumber: number) => {
      ...
      pages.forEach(async (item: any, index: number) => {
        const textToSaveAsPage = formatAyah(item)
        const userPageNumber = pageNumber + index

        console.log(`---- Page N°${userPageNumber}`);

        const filePath = path.join(__dirname, './quran/normal-pages/', `page${userPageNumber}.json`);
